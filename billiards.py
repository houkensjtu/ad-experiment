...
x = vec()
x_inc = vec()  # for TOI, Time Of Impact
v = vec()
impulse = vec()
...
radius = 0.03
# elasticity = 0.3
elasticity = ti.field(dtype=real, shape=(), needs_grad=True)  # Elasticity as a global 0D field
...
def collide_pair(t, i, j):
    ...
    if i != j:
        ...
        if dist_norm < 2 * radius:
            ...
            if projected_v < 0:
                imp = -(1 + elasticity[None]) * 0.5 * projected_v * dir
...
def advance(t: ti.i32):
    for i in range(n_balls):
        ...
        x[t, i] = x[t - 1, i] + dt * v[t, i] + x_inc[t, i]  # What is x_inc?
...
def compute_loss(t: ti.i32):
    loss[None] = (x[t, target_ball][0] - goal[0])**2 + (x[t, target_ball][1] -
                                                        goal[1])**2
    ''' Compute the sum of distance from target to each ball
    for i in range(n_balls):
        loss[None] += (x[t, i][0] - goal[0])**2 + (x[t, i][1] - goal[1])**2
    '''

@ti.kernel
def clear_loss():
    ''' Separate statement from for-loops
    '''
    loss[None] = 0.0
...
def optimize():
    ...
    for iter in range(400):
        ...
        print(f'>>> Iter={iter} Loss={loss[None]:6.2e} elasticity.grad={elasticity.grad[None]}')
